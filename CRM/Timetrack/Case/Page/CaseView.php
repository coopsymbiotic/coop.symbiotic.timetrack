  ...
  public function caseSummary($case_id) {
    ...
    if ($dao->fetch()) {
      ...
      foreach ($actionsreg as $key => $action) {
        $actions_html .= "<a href='{$action['url']}' class='button no-popup' style='display: inline-block; padding-left: 1em;'><span><div class='{$action['classes']}'></div>{$action['label']}</span></a>";
      }
