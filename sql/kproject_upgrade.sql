--
-- This is an upgrade procedure for those upgrading from Koumbit's kproject.
-- It has not been extensively tested. Your mileage may vary.
--
...
--
-- convert uid to contact_id
--
ALTER TABLE kpunch ADD `contact_id` int(10) unsigned DEFAULT NULL;
UPDATE kpunch
  JOIN civicrm_uf_match uf ON (kpunch.uid = uf.uf_id)
   SET kpunch.contact_id = uf.contact_id;

ALTER TABLE kpunch ADD KEY `contact_id` (`contact_id`);
ALTER TABLE kpunch ADD CONSTRAINT `FK_civicrm_contact_id` FOREIGN KEY (`contact_id`) REFERENCES `civicrm_contact` (`id`) ON DELETE SET NULL;
